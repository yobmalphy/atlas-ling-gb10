...
// V: extract v_dim portion from kv_expanded per head and zero-pad each
// head to hd.  The generic prefill-attention kernel strides V by hd, just
// like Q/K.  Packing Ling's 128-wide values back-to-back while passing
// hd=192 shifts every head after head 0 and makes the kernel read stale
// scratch data.
...
//   blockIdx.y==1: extract/pad V [nkv * hd elements per token]
extern "C" __global__ void mla_kv_assemble_batched(
    ...
    __nv_bfloat16* __restrict__ v_out,               // [N, nkv * hd], V then zeros
    unsigned int nkv,
    unsigned int nope,
    unsigned int v_dim,
    unsigned int rope,
    ...
    if (blockIdx.y == 0) {
        ...
        // Extract and right-pad V: [nkv, hd].  Attention produces an
        // hd-wide result; Ling compacts its first v_dim values afterward.
        unsigned int v_total = nkv * hd;
        for (unsigned int idx = threadIdx.x; idx < v_total; idx += blockDim.x) {
            unsigned int head = idx / hd;
            unsigned int dim = idx % hd;
            __nv_bfloat16 val = __float2bfloat16(0.0f);
            if (dim < v_dim) {
                // V is at offset nope within each head's
                // (nope + v_dim) source block.
                val = kv_expanded[(unsigned long long)t * kv_expanded_stride
                    + head * (nope + v_dim) + nope + dim];
            }
            v_out[(unsigned long long)t * nkv * hd + idx] = val;
        }
    }
}
